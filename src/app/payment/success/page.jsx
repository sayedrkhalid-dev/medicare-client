...
function CheckoutSuccessContent() {
  ...
  const maxPollAttempts = 15; // 15 attempts (30 seconds total) for robust network processing
  ...
  useEffect(() => {
    ...
    async function pollSessionVerification() {
      try {
        ...
        // Clean extraction handling dynamic Axios/Fetch wrappers securely
        const resultData = response?.data?.data || response?.data;
        const paymentStatus = resultData?.status?.toLowerCase();

        // ✅ FIXED: Changed misspelled "succeded" to "succeeded" / "success"
        if (
          response?.success &&
          (paymentStatus === "succeeded" || paymentStatus === "success")
        ) {
          // ✅ FIXED: Target direct schema output layout maps
          setAppointmentDetails(resultData);
