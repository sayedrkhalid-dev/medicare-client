...
export default function UserDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      ...
      try {
        ...
        toast.error(error?.message || "Couldn't load this user's profile.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // Close self-suspend modal on Escape; lock body scroll while open
  useEffect(() => {
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      // Check specifically for self-suspension server message first, then
      // fall back to status codes so we don't catch unrelated 400s.
      const errorMessage =
        error?.response?.data?.message || error?.message || "";
      const lowerMessage = errorMessage.toLowerCase();

      const isSelfSuspend =
        lowerMessage.includes("cannot suspend your own") ||
    ...
    <div className="space-y-4 max-w-7xl p-4 mx-auto min-h-screen bg-slate-50 dark:bg-[#090d16] text-slate-600 dark:text-slate-300 transition-colors duration-200">
      ...
      <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-2xl p-5 md:p-6 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3 text-sm border-t border-slate-100 dark:border-slate-800 pt-3">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between items-start sm:items-center py-2 md:col-span-2 gap-1 sm:gap-4">
            ...
            <span className="font-semibold text-slate-700 dark:text-slate-300 text-left sm:text-right max-w-xl">
      ...
      <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-2xl p-5 md:p-6 shadow-sm space-y-4">
        ...
        <p className="text-xs text-slate-400 dark:text-slate-500 leading-relaxed max-w-2xl select-none">
          ...
          account accessibility across the platform. Execute status changes with
          care.
        </p>
          ...
          <div className="relative w-full max-w-md transform overflow-hidden rounded-2xl bg-white dark:bg-[#111827] border border-rose-200/40 dark:border-rose-950/40 p-6 text-left shadow-xl transition-all space-y-4">
            <div className="flex items-start gap-3.5">
              ...
              <div className="space-y-1.5 flex-1">
                ...
                >
                  You cannot suspend your own administrative profile. This would
                  immediately terminate your active session and revoke all
